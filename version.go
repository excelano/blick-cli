...
// resolveVersion returns the version blick reports.
//
...
func pickVersion(stamped string, info *rtdebug.BuildInfo) string {
...
func versionLine() string {
	...
	if commit == "none" || date == "unknown" {
		return fmt.Sprintf("blick %s", v)
	}
	return fmt.Sprintf("blick %s (%s, %s)", v, commit, date)
}
