...
func TestPickVersion(t *testing.T) {
	release := &rtdebug.BuildInfo{} // goreleaser stamps the version; build info is irrelevant
	install := &rtdebug.BuildInfo{Main: rtdebug.Module{Version: "v1.4.0"}}
	devBuild := &rtdebug.BuildInfo{
		Main: rtdebug.Module{Version: "1.4.1-0.20260808004201-d7b9d3ff5354+dirty"},
		Settings: []rtdebug.BuildSetting{
			{Key: "vcs", Value: "git"},
	...
	for _, tc := range []struct {
		...
		{"module reporting devel keeps the placeholder", versionPlaceholder, &rtdebug.BuildInfo{Main: rtdebug.Module{Version: "(devel)"}}, versionPlaceholder},
	} {
