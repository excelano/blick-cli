...
func TestSearchKQL(t *testing.T) {
	cases := []struct {
		...
		{"embedded quote escaped", searchQuery{subject: `say "hi"`}, `subject:"say \"hi\""`},
		{"embedded quote no whitespace escaped", searchQuery{from: `a"b`}, `from:a\"b`},
		{"backslash escaped", searchQuery{text: []string{`a\b`}}, `a\\b`},
